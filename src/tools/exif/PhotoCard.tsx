  ...
  return (
    ...
    >
      <div className="relative bg-frame leading-[0]">
        ...
        {located && (
          <span
            className="absolute top-[0.7rem] right-[0.7rem] z-[2] inline-flex items-center gap-[0.4rem] font-mono text-2xs tracking-[0.02em] px-[0.6rem] py-[0.24rem] rounded-full bg-[rgba(20,18,15,0.55)] text-white backdrop-blur-[4px] leading-[1.4]"
            title="Geotagged"
          >
        ...
        {url && !decodeError ? (
          ...
          <div className="w-full aspect-[4/3] flex items-center justify-center bg-frame text-muted text-xs text-center p-4 font-mono leading-[1.5]">
            {decodeError
      ...
      <div className="flex flex-col gap-[0.7rem] flex-1 p-[1.1rem_1.2rem_1.2rem]">
        ...
        {exif === undefined ? (
          <p className="m-0 text-sm text-muted">Reading EXIF…</p>
        ) : isEmptyExif(exif) ? (
          <p className="m-0 text-sm text-muted">No EXIF metadata.</p>
        ) : (
          <div className="flex flex-col gap-[0.3rem]">
            {camera && (
              <p className="m-0 text-sm font-medium text-ink truncate" title={camera}>
                {camera}
              </p>
            )}
            {exposure && (
              <p className="m-0 font-mono text-xs tabular-nums text-ink-soft">
                {exposure}
              </p>
            )}
