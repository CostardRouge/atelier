...
export const note =
  'm-0 px-2.5 py-2 rounded-paper border border-line bg-paper text-xs text-ink-soft';
...
export function chipClass(selected: boolean): string {
  return `px-2 py-1.5 rounded-paper border text-center cursor-pointer text-xs transition-colors ${
    selected
