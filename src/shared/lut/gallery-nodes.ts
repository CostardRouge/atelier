 ...
 * draw a screen.
 *
 * Pure and DOM-free: the built-in manifest, the film stocks and the packs in,
 * a flat list of nodes and their items out. What an item's picture COSTS is
 * the caller's problem — a pack look carries a thumbnail baked at import, a
 * built-in is baked on the spot.
 */
...
import { flattenNodes, looksUnder, visibleLooks, type LutPackIndex } from './lut-pack';
...
export interface GalleryItem {
  ...
  id: string;
  name: string;
  /** Baked at import — a pack look draws without resolving a lattice. */
  thumb?: string;
  /** For everything else: the cube, fetched or generated, baked on the sample. */
...
export function galleryNodes(
  ...
  for (const pack of packs) {
    ...
    nodes.push({
      ...
      depth: 0,
      pack,
      items: rootLooks.map((l) => ({
        id: packPickId(pack.id, l.id),
        name: l.label,
    ...
    for (const { node, depth } of flattenNodes(pack.tree)) {
      ...
      nodes.push({
        ...
        items: (direct.length ? direct : branch).map((l) => ({
          id: packPickId(pack.id, l.id),
          name: l.label,
          ...(l.thumb ? { thumb: l.thumb } : {}),
        })),
