...
/**
 * Where it lands on a phone or a tablet (`lib/device-class.ts`): two and a
 * half DJI DNGs. Safari says nothing about its memory, so before this an
 * iPhone took the desktop default — half a gigabyte of held originals
 * beside the decoder's own heap, on a tab that is killed at a fraction of
 * that.
 */
export const CONSTRAINED_HELD_CEILING = 192 * MIB;
 ...
 * kept between 256 MiB and 1 GiB — unless the device is CONSTRAINED, which
 * takes its own flat ceiling whatever the browser reports (Chrome on Android
 * says 8 GiB of a phone whose tab is given far less). A quarter, because the
 * tab also feeds a stage at its pixel budget, a decoder and an export from
 * the same memory.
 */
export function heldCeiling(deviceMemoryGiB: number | null | undefined, klass: DeviceClass = 'roomy'): number {
  if (klass === 'constrained') return CONSTRAINED_HELD_CEILING;
