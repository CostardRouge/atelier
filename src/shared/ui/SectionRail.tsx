...
export default function SectionRail({ bar, onLibrary }: SectionRailProps) {
  ...
  return (
    ...
    >
      {onLibrary && (
        <>
          {/* Never marked, like every other cell here: what it opens covers
              this bar, so a mark would only ever be seen once the panel has
              been dismissed — which is precisely when it is a lie. */}
          <button
            type="button"
            onClick={onLibrary}
