...
export function attachMotion(cues: Cue[]): void {
  for (let i = 0; i < cues.length; i++) {
    ...
    if (i > 0) {
      ...
      const prev = cues[ref];
      motion = motionOver(prev, cur, cur.start - prev.start);
    }
 ...
 * case.
 */
function attachLead(cues: Cue[]): void {
  if (cues.length < 2) return;
  const openEnd = cues[0].start + WINDOW_S;

  for (let i = 0; i < cues.length; i++) {
    const cur = cues[i];
    if (cur.start >= openEnd) break;

    // Reference = the earliest cue at least WINDOW_S ahead — the exact mirror
    // of the look-back — falling back to the last cue of a very short track.
    let ref = firstIndexAtOrAfter(cues, cur.start + WINDOW_S);
    if (ref < 0) ref = cues.length - 1;
    if (ref <= i) continue;

    const known = cur.derived ?? {};
    const lead = motionOver(cur, cues[ref], cues[ref].start - cur.start);
