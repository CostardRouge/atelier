...
- **Atelier** — a local-first suite of browser tools for photo/video captures (DJI telemetry, LUT grading, culling, scopes, compositing). React 18 + TypeScript 5.7 + Vite 6 + Tailwind v4, no backend: everything runs client-side and no file ever leaves the machine. Deployed to GitHub Pages by `.github/workflows/deploy.yml` on every push to `main`. Package manager: **npm** — the lockfile is `package-lock.json` (it is the only one; keep it that way).
- Commands: `npm run dev` (Vite dev server), `npm run build` (`tsc -b && vite build` → `dist/`), `npm test` (Vitest, `src/**/*.test.ts`), `npm run typecheck`, `npm run lint` (ESLint), `npm run preview`. CI (`.github/workflows/ci.yml`) runs typecheck + lint + test + build on every push and pull request — run the same four before you call a task done.
...
2. **Select only the task's files**:
   ...
   - Never stage: `.env` and any secret file, `.idea/`, `.vscode/`, `dist/`, `node_modules/`, `.mockups/`, `*.tsbuildinfo`, `.DS_Store` — unless the task is explicitly about them. If one of these turns out to be *tracked*, say so: it must be untracked and gitignored, not carefully avoided at every commit.
...
- **Title**: English imperative, clear sentence, ≤ 72 chars, no `feat:`-style prefix, no trailing period. A `Tool:` scope prefix is used when the change is confined to one tool. Real examples from this repo: `Fix black map: don't let MapLibre's position:relative collapse the container`, `Composer export: preserve the map scale/zoom from the preview`, `Let users complete incomplete video/SRT pairs`.
...
- The requested code is written **and** verified: `npm run typecheck`, `npm run lint`, `npm test` and `npm run build` green (that is exactly what CI gates), or a check in the browser via `npm run dev` when the change is visual or depends on real media.
...
1. **Reach `second-brain`.** Locally it is a sibling directory
   ...
   its environment**.
2. Once it is reachable, edit `projets/atelier/README.md` and commit it there,
...
3. Only if it stays unreachable — the tool refused, or does not exist — write
   ...
   repository's pull request, where it can be found and collected later, and
       ...
       projets/atelier/README.md — point: YYYY-MM-DD
...
- **Never hardcode the GitHub Pages base path.** `vite.config.ts` derives it from `GITHUB_REPOSITORY` (with a local fallback and a `BASE_PATH` override); a hardcoded value already drifted on a repo rename and 404'd every asset on the deployed site.
- **Adding or removing a `.cube` LUT under `public/luts/` needs no code change.** The `virtual:luts` Vite plugin scans that folder at build/dev time; the dev server re-scans on add/unlink. Do not maintain a manual list.
- **`npm run build` type-checks first** (`tsc -b`), and `tsconfig.app.json` has `noUnusedLocals`/`noUnusedParameters` on: a leftover import or parameter fails the build, not just the lint.
- Pure logic (`*-parser.ts`, `*-plan.ts`, `flight-path.ts`, `scopes.ts`, `compose-layout.ts`, …) is DOM-free and unit-tested in a node environment: keep new pure logic in its own module with a `.test.ts` beside it rather than inside a component. `shared/` must never import from `tools/`.
