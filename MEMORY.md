...
- 2026-09-03 (rev. 2026-09-06) — **Winnow's timeline is being built; Atelier's client-only half is under way.** The brief is `docs/winnow-timeline.md` (companion to `winnow-bridge.md`, written before the spec exists, every Winnow-side claim marked as an assumption). Phases T0 (`TripDoc.sourceId` v10 + `TripStage.origin`, the file split) and T1 (`shared/roadtrip/timeline-import.ts`: mapping, span, diff, apply — pure and tested) are **built**; nothing calls T1 yet. T2–T4 wait on Winnow's `chapter_id` filter, `capabilities.media.timeline` and the spec's answers to §7; when it lands, the client normalises the wire shape into `TimelineChapter` and the two T3 panels are wiring over T1.
