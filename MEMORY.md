...
- 2026-09-03 (rev. 2026-09-06) — **Winnow's timeline is being built; Atelier's client-only half is under way.** The brief is `docs/winnow-timeline.md` (companion to `winnow-bridge.md`, written before the spec exists, every Winnow-side claim marked as an assumption). Phases T0 (`TripDoc.sourceId` v10 + `TripStage.origin`, the file split), T1 (`shared/roadtrip/timeline-import.ts`: mapping, span, diff, apply — pure and tested) and T2 (browse by leg in `WinnowBrowser`, gated on `capabilities.media.timeline`) are **built on Atelier's side**. T2 rests on an ASSUMED wire shape held in one function, `chapterFromWire` (`client.ts`) — when Winnow's spec lands, re-check it against §7 of the brief and change that function alone. **Unverified against a live instance**: no Winnow serves a timeline yet, so the tab has only been exercised through the specs.
