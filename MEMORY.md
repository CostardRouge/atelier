...
- 2026-08-31 — The custom domain is **live** and `deploy.yml` pins `BASE_PATH=/` (see `deployment.md`), so Atelier and Winnow are now same-site and the bridge needs **no credential system** in the near term. What is left of that prerequisite is on Winnow's side: a CORS allowlist naming exactly that origin, plus `Access-Control-Expose-Headers: Content-Range, Content-Length, Accept-Ranges` — omit the last and Range fails silently (`docs/winnow-bridge.md` §3.3). Two user-visible links are now stale and should follow the move: `REPO_URL` in `src/app/site.ts` and the README's `costardrouge.github.io/atelier/`.
