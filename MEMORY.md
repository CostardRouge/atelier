...
- A status whose sentence nobody controls the length of belongs in a header row as a PILL of the same family — a dot and one word, the sentence and its verbs in a popover on click or hover — never as a row of its own; on a phone the word survives only where the state waits on the author — `frontend.md`.
...
- A Winnow thumbnail is ONE component (`shared/sources/winnow/WinnowThumb.tsx`) that RETRIES: an `<img>` fires `error` once and stays black, and a CORS-poisoned `immutable` cache entry never heals — only the `?retry=N` URL defeats it — `architecture.md`.
...
- 2026-09-13 — **A photo editor is designed, not built** (`docs/photo-develop.md`). The suite has a look engine (`.cube` LUTs) and no correction engine: no control anywhere moves a picture's exposure, and a DNG is a file three surfaces say they cannot open, with no decoder in the dependency list. The brief settles the shape (a develop stage baked into the one cube, a RAW developed at decode, one sheet for both tools) and leaves seven choices to the maintainer — the decoder (`libraw-wasm` gated on a spike with his own DJI and iPhone DNGs, our own DNG reader as the fallback), the histogram, whether a developed picture goes home to Winnow as an edit, and when the bridge carries it. Nothing in `src/` has changed; P1 (the pure engine) is the first commit once he answers.
