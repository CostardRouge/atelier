...
- 2026-09-03 (rev. 2026-09-06: the timeline has shipped; `TripDoc.sourceId` landed as P0 of `docs/roadtrip-persistence.md`) — **Winnow's timeline is being built and nothing on Atelier's side is.** The brief that anticipates it is `docs/winnow-timeline.md` (companion to `winnow-bridge.md`, written before the spec exists, every Winnow-side claim marked as an assumption). Still worth doing before the spec lands because it is correct under every version of it: `TripStage.origin` (phase T0, the half of it `sourceId` did not cover) and the pure `timeline-import.ts` mapping and diff (T1).
