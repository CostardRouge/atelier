...
- Looking at ONE picture is the OTHER zoom: the lightbox's deck is a transform, not a scroll box, because it must follow a finger past its own edges — fit to 8×, drag pans when zoomed and swipes when not, and a wheel gesture ends on 140ms of quiet — `frontend.md`.
