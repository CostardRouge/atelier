...
- 2026-08-21 — The library row's name/facts column is **32 px** at the 288 px sidebar width (measured in Chromium against the built CSS: 288 − padding − checkbox 15 − thumb 80 − kind chip ~69 − remove 20 − four 10 px gaps). Names truncate to "DJI…" and the facts line shows nothing, which is why the cadence had to go on the thumbnail as a scrim chip. Freeing it means moving or shrinking the kind chip — a maintainer call, since that chip's colour coding is how a row's kind reads at a glance.
