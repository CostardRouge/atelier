...
- Exports **already** carry a correct `colr` bt709 tag, via encoder metadata mp4-muxer turns into the box — measured; do not "add" tagging — `media-pipeline.md`.
...
- 2026-08-21 — Four latent defects surveyed but not fixed, in rough order of how baffling they are when they bite: (a) the LUT shader hard-codes `clamp(rgb, 0, 1)` and ignores `DOMAIN_MIN`/`DOMAIN_MAX`, which `sampleLut` honours — so a user-uploaded cube with a non-unit domain renders one way at 100% strength (fast path returns the source cube) and another the moment the slider moves or a second layer is added; (b) the 8-bit LUT fallback clamps LUT *output* to [0,1], destroying the shipped DJI cube's `#Not-Clipped.` highlight rolloff on a GPU without `OES_texture_float_linear` — now nearly free to fix, since the tetrahedral path uses `texelFetch`, which needs no linear filtering; (c) mp4-muxer will silently write matrix=0 (Identity) if a browser ever reports an unmapped colour space (see `media-pipeline.md`); (d) the LUT bake still runs synchronously during render, ~28 ms per strength-slider step, because the slider has no local state or debounce. None of the four costs anything at export time.
