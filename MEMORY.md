...
- 2026-09-03 (rev. 2026-09-06) — **Winnow's timeline is being built; Atelier's client-only half is under way.** The brief is `docs/winnow-timeline.md` (companion to `winnow-bridge.md`, written before the spec exists, every Winnow-side claim marked as an assumption). Phase T0 (`TripDoc.sourceId` v10 + `TripStage.origin`, the file split) is **built**; T1 (the pure `timeline-import.ts` mapping and diff) is next and also spec-independent. T2–T4 wait on Winnow's `chapter_id` filter, `capabilities.media.timeline` and the spec's answers to §7.
