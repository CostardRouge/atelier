...
- 2026-09-03 (rev. 2026-09-06) — **Winnow's timeline is being built; Atelier's client-only half is under way.** The brief is `docs/winnow-timeline.md` (companion to `winnow-bridge.md`, written before the spec exists, every Winnow-side claim marked as an assumption). Phases T0 (`TripDoc.sourceId` v10 + `TripStage.origin`, the file split), T1 (`shared/roadtrip/timeline-import.ts`: mapping, span, diff, apply — pure and tested), T2 (browse by leg in `WinnowBrowser`, gated on `capabilities.media.timeline`) and T3 (`TimelineImportPanel`: seed / complete a trip, the two link routes, the connect `return`) are **built on Atelier's side**. They rest on an ASSUMED wire shape held in one function, `chapterFromWire` (`client.ts`) — when Winnow's spec lands, re-check it against §7 of the brief and change that function alone. **Verified only against a stubbed instance** (recipe in `testing.md`): no Winnow serves a timeline yet. Still open on Winnow's side: the "Make a Road Trip from this leg" verb pointing at `#/roadtrip/new?source=<host>&chapters=<id>`.
