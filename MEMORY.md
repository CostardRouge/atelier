...
- On a phone the library **docks and takes a share of the height** rather than covering the stage, so choosing a picture is a comparison and not a guess-and-check: two rests (half to choose at, a one-row strip to compose at), tiles whose big target SETS the picture while a corner verb looks at it. A stage above a dock must FLEX, and only if its whole chain does — `frontend.md`.
