...
**Phase T0 is built (2026-09-06)**: `TripDoc.sourceId` (v10, the migration files every older trip under `local`, `createTripDoc` takes it as a trailing argument) and `TripStage.origin` (`StageOrigin`: the instance's host as `sourceIdFor()` mints it, the chapter's own id, an optional `revision`, `importedAt`). **The split in `.roadtrip.json` is deliberate and asymmetric**: `sourceId` is stripped both ways (an imported trip belongs to the source that imports it — `tripDocFromFile` takes the receiving source), while `origin` TRAVELS — the one pointer outside the browser the file keeps, because a host + that host's own chapter id means the same thing on another machine connected to the same Winnow, and dangles harmlessly where it is not. Never reuse a chapter id as `TripStage.id`, and never let anything but the reconcile diff read `origin`: a seeded trip must open identically with the instance gone.
