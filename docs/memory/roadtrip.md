...
**Verified only against a stubbed instance** (Playwright route on `winnow.test`, this container): the gallery's remote list and its three header states, create-on-remote, the pill's sentences, conflict → keep mine / take theirs, gone. **Not verified**: the deployed pair (`localhost` is cross-SITE to `winnow.steeve.website`, the cookie cannot travel) — §10 of the brief is the script to run there, and P2 must be deployed first.
