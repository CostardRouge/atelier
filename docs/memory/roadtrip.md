...
## The piece editor is tabs, in Road Trip's nouns (2026-09-06)

**Decision, from `docs/roadtrip-editor.md`.** The 16-section one-at-a-time accordion is gone; the inspector is a tab bar over a 22rem column — Content · Style · Picture · Deck · Export (Grade joins when grading lands) — with the tab in `useState`, never in the route (the route says WHERE you are, the tab says what you look at). Tab bodies are one file each under `tools/roadtrip/panels/`; `PostEditor` keeps state, the stage, the transport and the bar, and three hooks hold what used to inflate it: `use-badge-clock` (the transport), `use-post-exports` (PNG deck + hook clip, one progress line and one note), `use-slide-library` (the Library ↔ slide sync and its restore-once trap). Six tabs `flex-wrap` into two rows; do not `flex-1` them. **Wide, the editor is a two-column grid** (stage spanning both rows on the left, the piece's header atop the inspector on the right) so nothing sits above the picture — a header row over both columns cost the portrait preview 60px; how the stage then fills that height is in `frontend.md`.

**Allocation rule, re-applied while moving every control** (the invariant from «A panel that belongs to the PIECE»): the day, the frame, the Studio bridge, the per-kind defaults and the closing card show on EVERY slide; the badge's own pieces, counter, time, placement, shades and duration on the hook only; a caption on a content slide only. The hook clip export and the bridge therefore read the HOOK's file and dimensions (`hookFile`, `hookInfo`, kept apart from the open slide's) so they still work from a carousel's second picture — before the split they silently read the open slide's. An export started from the header switches to the Export tab so its note is seen.
