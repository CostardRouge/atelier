...
- **A Road Trip kept on a connected Winnow** (`shared/roadtrip/trip-remote.ts`, since 2026-09-06) — the one thing Atelier ever WRITES to a server, and only when the author picked that instance on the "Keep on" picker (creation, import, move). What travels is the trip document (place names, dates, badge copy, media refs by hash — never a picture, never a thumbnail), under the user's own account there (`/api/apps/atelier/docs`), pushed after ~5 s idle, on tab hide, on leaving the trip and on "Save now"; opening the trip reads it back with `If-None-Match`, and the gallery lists what the account keeps there. A trip kept locally makes no request at all. Stated in the README's Sources section and its network callout. Design and limits: `docs/roadtrip-persistence.md`.
