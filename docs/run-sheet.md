...
| ⛔ | **R4** | The export follows | `use-roll-export.ts` resolves the rendition the same way; the *Delivers* row names it. **Waiting on §13.2** — what `Proxies` means over a picture developed from its sensor. | the pane, two runs |
...
| ◻ | **R6** | The lightbox switcher | The shell's media preview offers the capture's other files (his B7). Scope open: read-only, or may it write the stored choice? | the pane |
