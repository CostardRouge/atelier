...
// =============================================================================
// MARK: - CROP — owned by the Crop task (`Develop/Crop/*`): the classic crop's
// zone on the stage, border, perspective (keystone), lens + its profile.
// Interim: Develop v0's aspect · straighten · mirror. Delete when it lands.
// =============================================================================

struct CropSectionPlaceholder: View {
    @Bindable var editor: RollEditor
    @Environment(\.palette) private var palette

    private var framing: Framing { editor.picture?.framing ?? .default }

    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            DevelopSection(id: "crop", title: "Crop") {
                VStack(alignment: .leading, spacing: 12) {
                    Picker("Aspect", selection: editor.binding(\.aspect, fallback: "original")) {
                        ForEach(AspectOption.all) { option in
                            Text(option.id == "original" ? "Original" : "\(option.id) · \(option.label)").tag(option.id)
                        }
                        if let aspect = editor.picture?.aspect, !AspectOption.all.contains(where: { $0.id == aspect }) {
                            Text(aspect).tag(aspect)
                        }
                    }
                    Eyebrow("Straighten")
                    HStack {
                        Slider(value: Binding(get: { framing.rotation },
                                              set: { degrees in setFraming { $0.rotation = wrapDegrees(degrees) } }),
                               in: -45...45, step: 0.1)
                        Text(String(format: "%+.1f°", framing.rotation))
                            .font(Brand.mono(12))
                            .foregroundStyle(framing.rotation == 0 ? palette.muted : palette.ink)
                            .frame(width: 56, alignment: .trailing)
                    }
                    HStack(spacing: 8) {
                        Button("Left ↔ right") { setFraming { $0 = flipFraming($0, axis: "x") } }
                        Button("Top ↔ bottom") { setFraming { $0 = flipFraming($0, axis: "y") } }
                        Spacer()
                        Button("Reset crop") {
                            editor.update { r in
                                guard let id = editor.openId else { return r }
                                return patchPicture(r, id) { $0.framing = nil; $0.aspect = "original" }
                            }
                        }
                    }
            ...
            ApplySection(editor: editor, verbs: editor.cropApplyVerbs, title: "Apply crop to…", id: "crop-apply")
            ComingSection(id: "crop-rest", title: "Border · perspective · lens",
                          text: "The crop's zone drawn over the whole picture, the border, the keystone and the lens with its measured profile — coming with the Crop task. What the web app set is kept.")
        }
    }

    private func setFraming(_ change: (inout Framing) -> Void) {
        guard let id = editor.openId else { return }
        var f = framing
        change(&f)
        let next: Framing? = isDefaultFraming(f) ? nil : f
        editor.update { patchPicture($0, id) { $0.framing = next } }
    }
}

/// The crop's zone over the whole picture, under a veil.
struct CropStageOverlay: View {
    let context: StageOverlayContext
    var body: some View { PendingOverlayChip(text: "the crop's zone is coming — aspect, straighten and mirror are in the inspector") }
}
