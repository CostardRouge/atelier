...
| ⏳ the task pill for a transfer | a caller wraps the client call in `WinnowTransfer.$progress` and reports into the kernel's `TaskRegistry` |

**Counts**: 62 rows — 47 ✅ (4 of them native-only), 9 ⏳, 6 ≠ (built differently on purpose).
